...
$sql = "SELECT * FROM journalist WHERE email= '$session_email' ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($name) && !empty($email)) {
        ...
        $sql_select_query = "SELECT email FROM journalist WHERE email = '$email' ";
        ...
        if (mysqli_num_rows($r) > 0) {
            ...
            $sql = "UPDATE journalist SET name = '$name', email = '$email', gender= '$gender' WHERE email='$_SESSION[email]' ";
            $result = mysqli_query($conn, $sql);
            if ($result) {
